...
Don't remember the flag's prefix, just a word buried in it? Type that word, no dashes, wherever it actually falls in the name. `tool admin<Tab>` (or `sensei args tool admin`) against a tool with `--count-admin`, `--list-admins`, `--admin-only`, and `--username` surfaces the first three, not the fourth — matched against each flag's own hyphen-separated words first (an exact word beats one that merely starts with it, which beats a bare substring), falling back to the description text only if nothing in the name itself matches. Same fallback logic covers subcommand names. This only kicks in once an exact prefix match comes back empty — `-s<Tab>` still narrows to `-sS`/`-sV`/etc. exactly as before, this doesn't change that.
